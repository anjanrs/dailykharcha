...
const accessPerUseGroupDetailPageMiddleware = ({
  getState,
  dispatch
}) => next => action => {
  if (
    ...
    action.detailName &&
    action.detailName === "accessperusergroup-detail" &&
    action.type === datadetailActionTypes.DETAIL_INIT_SUCCESS
  ) {
    Promise.all([
      getAccessTypesOptions("usergroup_access"),
...
export default accessPerUseGroupDetailPageMiddleware;
